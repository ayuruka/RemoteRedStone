...
public class WebServer extends NanoHTTPD {
    ...
    private Response handleApiRequest(String uri, IHTTPSession session) {
        ...
        try {
            if (Method.POST.equals(session.getMethod())) {
                if ("get-live-states".equals(action)) {
                    ...
                    try {
                        ...
                        return jsonResponse(400, "error", "Could not parse POST body for live-states.");
                    }
                    ...
                    try {
                        ...
                        return jsonResponse(400, "error", "Could not parse form data.");
                    }
                }
            }
            ...
            if ("update-group".equals(action)) {
                plugin.locationManager.updateGroup(decodeParam(params.get("groupId")), decodeParam(params.get("newName")), decodeParam(params.get("newMemo")));
                return jsonResponse(200, "success", "Group updated.");
            }
            if ("update-switch".equals(action)) {
                plugin.locationManager.updateSwitch(decodeParam(params.get("switchId")), decodeParam(params.get("newName")));
                return jsonResponse(200, "success", "Switch updated.");
            }
            if ("toggle-switch".equals(action)) {
                String switchId = decodeParam(params.get("switchId"));
                boolean isON = "set".equals(decodeParam(params.get("state")));
                Map<String, Object> loc = plugin.locationManager.getAllLocations().get(switchId);
                if (loc != null) {
                    plugin.setSwitchBlock(loc.get("world").toString(), loc.get("x").toString(), loc.get("y").toString(), loc.get("z").toString(), isON);
                    plugin.locationManager.updateLocationState(switchId, isON ? "ON" : "OFF");
                    return jsonResponse(200, "success", "Toggled switch.");
                }
            }
            if ("remove-switch".equals(action)) {
                ...
                if (loc != null) {
                    ...
                    plugin.locationManager.removeLocation(switchId);
                    return jsonResponse(200, "success", "Removed switch.");
                }
            }
            if ("add-group".equals(action)) {
                plugin.locationManager.addGroup(decodeParam(params.get("groupName")), decodeParam(params.get("memo")), decodeParam(params.get("parentId")));
                return jsonResponse(200, "success", "Group '" + decodeParam(params.get("groupName")) + "' added.");
            }
            if ("remove-group".equals(action)) {
                plugin.locationManager.removeGroup(decodeParam(params.get("groupId")));
                return jsonResponse(200, "success", "Group and its switches removed.");
            }
            if ("toggle-group".equals(action)) {
                plugin.setGroupState(decodeParam(params.get("groupId")), "set".equals(decodeParam(params.get("state"))));
                return jsonResponse(200, "success", "Group state changed.");
            }
            if ("add-switch".equals(action)) {
                plugin.locationManager.addLocation(decodeParam(params.get("name")), decodeParam(params.get("world")), Integer.parseInt(params.get("x")), Integer.parseInt(params.get("y")), Integer.parseInt(params.get("z")), decodeParam(params.get("group")));
                plugin.setSwitchBlock(decodeParam(params.get("world")), params.get("x"), params.get("y"), params.get("z"), false);
                return jsonResponse(200, "success", "Added switch '" + decodeParam(params.get("name")) + "'.");
            }
        } catch (Exception e) {
    ...
    private Response jsonResponse(int status, String result, String message) {
        ...
        return newFixedLengthResponse(responseStatus, "application/json; charset=utf-8", "{\"status\":\"" + result + "\", \"message\":\"" + message + "\"}");
    }
    ...
    private String generateDashboard() {
        ...
        allGroups.entrySet().stream().filter(e -> !e.getValue().containsKey("parent")).forEach(parentEntry -> html.append(renderGroup(parentEntry.getKey(), parentEntry.getValue(), locations, worldOptions, childGroupsMap, false)));
        ...
        html.append("document.body.addEventListener('click', async e => { const btn = e.target; const action = btn.dataset.action; if (!action || btn.tagName !== 'BUTTON') return; e.preventDefault();");
        ...
        html.append("else return;if(confirmMsg&&!confirm(confirmMsg))return;const res=await fetch(url,{method:'POST',body:params});const data=await res.json();if(data.status==='success'){ if (action.startsWith('save')) { toggleEdit(btn.closest('[data-editable]'), false); } showMsg(data.message || 'Action successful!'); setTimeout(() => window.location.reload(), 500); } else { showMsg(data.message,true); }});");
        ...
        html.append("document.getElementById('group-search').addEventListener('input', e => { const query = e.target.value.toLowerCase(); document.querySelectorAll('.group').forEach(group => { const title = group.querySelector('h3').textContent.toLowerCase(); group.style.display = title.includes(query) ? '' : 'none'; }); });");
        html.append("setInterval(async () => { const switchRows = document.querySelectorAll('tr[data-switch-id]'); if (switchRows.length === 0) return; const switchIds = Array.from(switchRows).map(row => row.dataset.switchId); try { const res = await fetch('/api/get-live-states', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(switchIds) }); if (!res.ok) return; const liveStates = await res.json(); for (const [id, isON] of Object.entries(liveStates)) { const row = document.querySelector(`tr[data-switch-id='${id}']`); if (!row) continue; const onBtn = row.querySelector('.btn-on'); const offBtn = row.querySelector('.btn-off'); if (onBtn) onBtn.disabled = isON; if (offBtn) offBtn.disabled = !isON; } } catch (error) { console.error('Live update failed:', error); } }, 1000);");
        html.append("</script></div></body></html>");
        return html.toString();
    }

    private String renderGroup(String groupId, Map<String, Object> groupData, Map<String, Map<String, Object>> locations, String worldOptions, Map<String, List<Map.Entry<String, Map<String, Object>>>> childGroupsMap, boolean isSubGroup) {
        ...
        h.append("<div class='group-header display-view'><div><h3>").append(groupName).append("</h3>");
        if (!groupMemo.isEmpty()) h.append("<p class='group-memo'>").append(groupMemo).append("</p>");
        h.append("</div><div><button class='btn btn-edit' data-action='edit-item'>Edit</button></div></div>");
        ...
        h.append("<div class='actions'><button class='btn btn-on' data-action='toggle-group' data-group-id='").append(groupId).append("' data-state='set'>All ON</button><button class='btn btn-off' data-action='toggle-group' data-group-id='").append(groupId).append("' data-state='clear'>All OFF</button><button class='btn btn-del' data-action='remove-group' data-group-id='").append(groupId).append("'>Delete Group</button></div>");
        ...
        childGroupsMap.getOrDefault(groupId, new ArrayList<>()).forEach(childEntry -> h.append(renderGroup(childEntry.getKey(), childEntry.getValue(), locations, worldOptions, childGroupsMap, true)));
        if (!isSubGroup) {
